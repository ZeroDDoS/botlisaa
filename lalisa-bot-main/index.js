...
const instagram = 'https://instagram.com/farish.gt'; // Nama Instagramlu cok
const nomer = 'https://Wa.me/+6281282655499'; // Nomor whatsapplu cok
const aktif = 'Tergantung jaringan bos'; // Kapan bot lu aktif
const groupwa = 'soonnnnnnnnnnnnnnnnnnnnn/'; // OFFICIAL GRUP LU 1
const youtube = 'soonnnnnnnnnnnnnnnnnnnnn'; // OFFICIAL GRUP LU 2
//
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:3.0\n' 
            + 'FN:OWNER\n' // full name
            + 'ORG:Owner L4LISA Bot;\n' // the organization of the contact
            + 'TEL;type=CELL;type=VOICE;waid=6281282655499:+62 812-8265-5499\n' // WhatsApp ID + phone number
            + 'END:VCARD'
//
const
{
...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] L4LISA Ready scan now!`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @psicyolv`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @psicyolv`)
})
...
conn.sendMessage(id, 'Mangga akang teteh' ,MessageType.text);
}
else if (text == 'lisa'){
conn.sendMessage(id, 'Pacarnya Mas Wahyu' ,MessageType.text);
}
else if (text == 'Lisa'){
conn.sendMessage(id, 'Iya Kak Ada Apa' ,MessageType.text);
}
...
conn.sendMessage(id, 'Iyaa?, Ketik *.help* Untuk Menampilkan Menu' ,MessageType.text);
}
else if (text == '.ttp'){
conn.sendMessage(id, ' *COMMAND PREMIUM CHAT WA https://wa.me/6289655478810* ' ,MessageType.text);
}
else if (text == 'Woi'){
conn.sendMessage(id, 'Ya?, Ketik .Ketik *.help* Untuk Menampilkan Menu' ,MessageType.text);
}
else if (text == 'Eoy'){
...
conn.sendMessage(id, 'Nee Annyeong Kak' ,MessageType.text);
}
else if (text == 'annyeong'){
conn.sendMessage(id, 'Nee Annyeong Kak' ,MessageType.text);
}
else if (text == 'jmbt'){
conn.sendMessage(id, 'Jmbt Mu Lebat' ,MessageType.text);
}
else if (text == 'sepi'){
...
conn.sendMessage(id, 'Nih nomer pacarku...Eeh maksudku nomer owner/creator ku,chat ajah terus di save, nanti di back kok kak ><', MessageType.text)
}
...
axios.get(`https://arugaz.herokuapp.com/api/howbucins`).then((res) => {
	conn.sendMessage(id, 'Nih Kak', MessageType.text)
...
var tampilTanggal = "TANGGAL: " + hari + ", " + tanggal + " " + bulan + " " + tahun;
var tampilWaktu = "JAM: " + jam + ":" + menit + ":" + detik;
conn.sendMessage(id, intro.intro(id, XPTN, corohelp, tampilTanggal, tampilWaktu, instagram, nomer, aktif, groupwa, youtube) ,MessageType.text);
}
if (text == '.donasii'){
const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
...
conn.sendMessage(id, 'https://chat.whatsapp.com/Bz7XkMUkvUu2P3XLfz9UrN' ,MessageType.text);
}
     if (messageType == 'imageMessage')
   {
       let caption = imageMessage.caption.toLocaleLowerCase()
...
if (text.includes('lisaa')) {
 var nomor = m.participant
 const options = {
       text: `Iyaa Kak Ada Apa`,
       contextInfo: { mentionedJid: [nomor] }
 }
 conn.sendMessage(id, options, MessageType.text, { quoted: m })
}
